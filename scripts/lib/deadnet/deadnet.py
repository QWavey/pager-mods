...
try:
    ...
    # Pager-specific tweak: netifaces isn't in the OpenWRT opkg feed and
    # needs a C compiler to build from pip, which this device doesn't have.
    # get_gateway_ipv4() below already falls back to scapy's route table
    # when netifaces.gateways() fails, so make the import itself optional
    # too instead of hard-crashing the whole tool over one missing package.
    netifaces = None
