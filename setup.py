...
PAYLOAD_META = {
    ...
    "lan_sniffer": {"title": "LAN Sniffer", "author": "florian",
        "description": "Live LAN traffic view (auto-detected adapter, or bridge/tap both ports - full internet stays intact, self-heals eth0/br-lan if the bridge drops it) - timer or infinite duration, A pauses/resumes, B asks to stop, then offers to save the log. HTTP/DNS/creds flagged live. Logs progress right before the bridge call so a brief network-change pause isn't mistaken for a hang.", "version": "3.2"},
