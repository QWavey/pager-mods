...
- [x] **L10** Deep scan → exploit shortlist  (LanScan.sh — wired; add NSE shortlist)
- [ ] **L1** Auto-pwn on plug-in  (orchestrate recon→default-cred→loot)
- [ ] **L2** Responder + NTLM relay  (scapy LLMNR/NBNS/mDNS poisoner; relay = hard, phase 2)
- [ ] **L3** Default-cred auto-login + config exfil  (curl/py against discovered services)
- [ ] **L4** SMB share auto-loot  (needs smbclient — confirm opkg)
- [x] **L5** Selective MITM + inject  (dnsspoof.sh/deadnet.sh — wired via LAN theme)
- [ ] **L6** Pivot + reverse tunnel  (ssh -R / socat over mgmt radio)
- [ ] **L7** AD recon + Kerberoast  (impacket missing → scapy/manual, phase 2)
- [ ] **L8** Printer / IoT exploitation  (nmap NSE + targeted checks)
